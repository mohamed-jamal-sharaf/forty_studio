...
// Version: 3.1.0 - Premium Edition with Permissions & App Dropdown Fix
// Brand Color: #025074
// =====================================================
...
// =====================================================
// Load Apps Options for Select Field (Synchronous)
// =====================================================
let APPS_OPTIONS = '';
    ...
    frappe.call({
        ...
        callback: function(r) {
            if (r.message) {
                ...
                console.log('[APPS] Loaded options:', APPS_OPTIONS);
            }
        }
    });
}
...
function setAppFieldOptions(frm) {
    if (!APPS_OPTIONS) {
        console.log('[APPS] No options available');
        return;
    }
    ...
    console.log('[APPS] Field options set');
}
...
function setupEditorHandlers(frm) {
    ...
    wrapper.find('.btn-save').off('click').on('click', function () {
        saveFile(frm);
    });
...
// =====================================================
// Dialog Functions (with permission checks)
// =====================================================

function showCreateFileDialog(frm) {
    if (!frm.doc.app_name) {
        showAlert('Please select an app first', 'warning');
        return;
    }

    // Check permission
    if (!userPermissions.can_create) {
        showAlert('You don\'t have permission to create files', 'error');
        return;
    }
...
function showCreateFolderDialog(frm) {
    ...
    // Check permission
    if (!userPermissions.can_create) {
        showAlert('You don\'t have permission to create folders', 'error');
        return;
    }
...
function showDeleteFileDialog(frm) {
    ...
    if (!userPermissions.can_delete) {
        showAlert('You don\'t have permission to delete files', 'error');
        return;
    }
    ...
    try {
        ...
        dialog.$wrapper.find('.btn-restore-backup').on('click', async function () {
            try {
                ...
                if (error !== 'cancelled') console.error('[ERROR]', error);
            }
        });
    } catch (error) {
        console.error('[ERROR]', error);
...
// =====================================================
// Custom Buttons (with permission checks)
// =====================================================
...
frappe.ui.form.on('Forty Studio', {
    before_load: function(frm) {
        // Load apps synchronously before form renders
        console.log('[FORM] before_load - Loading apps options');
        loadAppsOptions();
    },

    setup: function(frm) {
        // Set app_name field options on meta level
        console.log('[FORM] setup - Setting meta options');
        if (APPS_OPTIONS) {
    ...
    onload: async function (frm) {
        console.log('[FORM] onload - Form loading...');
    ...
    file_path: function (frm) {
        ...
        // Refresh buttons when file is selected
        addCustomButtons(frm);
    }
});
...
window.addEventListener('beforeunload', () => {
    if (editor) {
        try { editor.dispose(); } catch (e) { console.error('Cleanup error:', e); }
    }
});

console.log('✓ Forty Studio with Permissions Loaded v3.1.0 - Capital Project');